...
class ViewController: TableDataNavigationViewController {
    ...
    func fetchData(){
        self.queue.async {
            self.dataDelegate.setData(data: self.datas as AnyObject?)
    ...
    override func tableDelegateConfig(){
        ...
        self.dataDelegate.rowObjectValueChangedCallback = {
            ...
            obj , oldObj , row , fieldName in
                ...
                self.datas[row] = obj as! Dictionary<String, Any>
        }
    }
    ...
    override func addNewData() {
        ...
        //定义一个默认数据
        let data: Dictionary<String,Any>  = [ "field":"","type":"","size":"8","primary":(0)]
        
        //插入的位置，默认数据插入当前页面的最后一行（也可以实现为插入当前选择的行的位置）
        let pos = (self.tableView?.numberOfRows)! - 1
        self.datas.insert(data, at: pos)
        //计算总数据的行
        self.computePageNumbers()
        //重新刷新页面
        self.pageManager.refreshCurrentPage( complete: {
                //设置光标焦点的位置
